import { Calendar, Clock, MapPin, User, Users, Share2, Download, ChevronRight } from 'lucide-react';
...
const EventDetailPage = () => {
  const event = {
    ...
    description: "Join us for our annual technology symposium featuring keynote speakers from leading tech companies, interactive workshops, and networking opportunities. This year's focus will be on Artificial Intelligence and its impact on various industries.",
    ...
    speakers: [
      { name: "Dr. Sarah Johnson", role: "AI Research Lead", company: "Tech Corp" },
      { name: "Prof. Michael Chen", role: "Department Head", company: "University" },
    ]
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-4 hover:text-primary-900 transition-colors duration-300">{event.title}</h1>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              ...
              <div key={index} className="flex items-center text-gray-600 p-3 rounded-lg hover:bg-white hover:shadow-md transition-all duration-300 group cursor-pointer">
                <div className="mr-2 text-gray-400 group-hover:text-primary-900 transition-colors duration-300">
                  {item.icon}
                </div>
                <span className="group-hover:text-gray-900 transition-colors duration-300">{item.text}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm hover:shadow-md transition-shadow duration-300">
              ...
              <div className="space-y-4">
                {event.schedule.map((item, index) => (
                  <div key={index} className="flex items-start p-2 rounded-lg hover:bg-gray-50 transition-colors duration-300 group cursor-pointer">
                    <div className="text-gray-600 font-medium w-24 flex-shrink-0 group-hover:text-gray-900">
                      {item.time}
                    </div>
                    <div className="text-gray-600 group-hover:text-gray-900">{item.activity}</div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm hover:shadow-md transition-shadow duration-300">
              ...
              <div className="space-y-4">
                {event.speakers.map((speaker, index) => (
                  <div key={index} className="border-b last:border-0 pb-4 last:pb-0 hover:bg-gray-50 p-3 rounded-lg transition-colors duration-300 cursor-pointer group">
